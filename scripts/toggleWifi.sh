...
# Grab the names of the adapters. We assume here that any ethernet connection name ends in "Ethernet" or "LAN"
eth_names=`networksetup -listnetworkserviceorder | sed -En 's/^\(Hardware Port: .*(LAN|Ethernet), Device: (en.)\)$/\2/p'`
...
if [ "$prev_eth_status" != "$eth_status" ]; then
    ...
    if [ "$eth_status" = "On" ]; then
        set_airport "Off"
        growl "Wired network detected. Turning Wi-Fi off."
    else
        set_airport "On"
        growl "No wired network detected. Turning Wi-Fi on."
    fi
